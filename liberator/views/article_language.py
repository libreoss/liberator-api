...
class ArticleLanguageViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, pk=None, language_pk=None):
        contents = self.queryset.filter(article=pk).filter(language=language_pk).order_by("date")
    ...
    def latest(self, request, pk=None, language_pk=None):
        ...
        for lang in languages:
            content = self.queryset.filter(article=pk).filter(language=lang).order_by("date").last()
